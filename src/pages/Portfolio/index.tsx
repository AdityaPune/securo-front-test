...
function StrategyPerformanceTable() {
  ...
  const strategyPerformanceList = useAppSelector(
    (state) => state.strategyPerformance.strategyPerformanceList
  );
  ...
  function createData(
    ...
    PnL: string,
    PnLPercentChange: string
  ) {
    return { name, price, holdings, twentyFourHours, PnL, PnLPercentChange };
  }

  const [MWIData, setMWIData] = useState(
    createData('Market Weighted Index', '0', '0', '0', '0', '0')
  );
  const [BNIData, setBNIData] = useState(
    createData('Blockchain Network Index', '0', '0', '0', '0', '0')
  );
  const [LCIData, setLCIData] = useState(
    createData('Low-risk Crypto Index', '0', '0', '0', '0', '0')
  );
  ...
  const helper = (index: number) => {
    ...
    const data = {
      name: productName,
      price: parseFloat(pricePerFullShare).toLocaleString('en-US', {
      ...
      PnL: parseFloat(profitAndLoss).toLocaleString('en-US', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
        style: 'currency',
        currency: 'USD',
              ...
              if (parseFloat(row.holdings) !== 0)
                return (
                  <TableRow key={row.name}>
                    ...
                    <TableCell
                      id={index === rows.length - 1 ? 'noBorder' : 'tableCells'}
                      component="th"
                      scope="row"
                    >
                      <Box
                      ...
                      >
                        ...
                        >
                          <img
                            src={require(`../../assets/images/products/${symbols[
                              index
                            ].toLowerCase()}.svg`)}
                            alt="product"
                            height="37"
                    ...
                    <TableCell
                      ...
                      id={index === rows.length - 1 ? 'noBorder' : 'tableCells'}
                    >
                      {parseFloat(row.holdings).toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                        style: 'currency',
                        currency: 'USD',
...
function PortfolioAllocationTable() {
  ...
  const portfolioAllocation = useAppSelector(
    (state) => state.strategyPerformance.portfolioAllocation.allocations
  );

  const [LCIData, setLCIData] = useState(
    createData('Low-risk Crypto Index', '0', '0')
  );
  const [MWIData, setMWIData] = useState(
    createData('Market Weighted Index', '0', '0')
  );
  const [BNIData, setBNIData] = useState(
    createData('Blockchain Network Index', '0', '0')
  );
  ...
  const rows = [MWIData, BNIData, LCIData, WalletData];

  const helper = (index: number) => {
    const { productName, changePercentage, allocation } =
  ...
  const series = [
    parseFloat(parseFloat(MWIData.allocation).toFixed(2)),
    parseFloat(parseFloat(BNIData.allocation).toFixed(2)),
    parseFloat(parseFloat(LCIData.allocation).toFixed(2)),
    parseFloat(parseFloat(WalletData.allocation).toFixed(2)),
  ];
  const labels = [
    MWIData.strategy,
    BNIData.strategy,
                          ...
                          if (parseFloat(row.allocation) !== 0)
                            return (
                              <TableRow key={index}>
                                ...
                                >
                                  {parseFloat(row.allocation).toFixed(2)}%
                                </TableCell>
                                <TableCell
                                  align="left"
                                  id={'noBorder'}
...
function Portfolio() {
  ...
  useEffect(() => {
    const fetchdata = async (period: string) => {
      ...
      const newChartData = portfolioChartData.map(
        (p: {
          changePercentage: string;
          timestamp: string;
          totalSharesInUsd: string;
          walletBalance: string;
        }) =>
          parseFloat(
            (
              parseFloat(p.totalSharesInUsd) + parseFloat(p.walletBalance)
            ).toFixed(3)
          )
      );
      const newTimestampData = portfolioChartData.map(
        (p: {
          changePercentage: string;
          timestamp: string;
          totalSharesInUsd: string;
  ...
  return (
    <Box
      mt={2}
      sx={{ width: '100%', marginLeft: '38px', marginRight: '30px' }}
    >
      <Card>
        <CardContent>
          <Box
          ...
          >
            ...
            {showCurrentBalance && currentBalanceData && (
              <Box
                className="current-balance-number"
                mb={1}
              >
