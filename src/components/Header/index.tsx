...
function Header() {
    ...
    useEffect(() => {
        if (userData !== undefined && userData.userPhotoUrl !== undefined && userData.userPhotoUrl !== null) {
    ...
    function GoBackComponent() {
        return <div className="flex-row align-items-center" id="go-back" onClick={() => navigate(parentPath)}>
    ...
    function AccountButtonComponent() {
        return <div id="account-action-button" onClick={() => setExpanded(!expanded)}>
            <div className={`content`}>
                <div className='flex-row align-items-center flex-space-between'>
                    <Avatar src={profilePicDisplayFile} alt="human" className={`${expanded ? "icon-align" : ""} `} style={{ marginRight: "4px", width: "32px", height: "32px" }} variant="rounded" />
                    <div id="name-container" className={`${expanded ? "expanded" : ""}`}>
                        ...
                        {expanded && <div className="expanded-container">
                            <div className='menu-item' style={{ borderTop: "0px" }} onClick={() => navigate("/my-account")}>
                                My Account
                            </div>
                            ...
                            <div className='menu-item special' onClick={() => handleLogout()}>
                                Logout
                            </div>
                        </div>}
                    </div>
    ...
    return <>
        {isAuthenticated && showStakedIcon && <Box style={{ marginLeft: "16px", marginTop: "16px" }}>
            <AppBar color="inherit">
                <Box display="flex" justifyContent="space-between" p={1} style={{ borderBottom: "1px solid #EEEEEE" }}>
                    <Box>
                        <img src={SecuroSymbol} style={{ height: "40px" }}></img>
                    </Box>
                    <Box mt={0.5}>
                        <img src={MenuIconSecuro} onClick={() => setOpenSide(!openSide)}></img>
        ...
        {!showStakedIcon &&
            <div className="header-container">
                <div className={`header-container-wrapper ${scroll ? expanded ? "scrolled-expanded" : "scrolled" : ''} ${expanded ? 'expanded' : ''}`}>
                    {!isAuthenticated && <CompanyLogo />}
                    {isAuthenticated && <>
                        {isChildrenPage ? <GoBackComponent /> : <Typography className={"header-container-title"}>{headerTitle}</Typography>}
                    </>}
...
export function Titles() {
    ...
    function GoBackComponent() {
        return <div className="flex-row align-items-center" id="go-back" onClick={() => navigate(parentPath)}>
            <img src={GobackIcon} alt="Go back" style={showStakedIcon ? { width: "18px", height: "18px" } : {}} />
            <span style={showStakedIcon ? { marginLeft: "8px", fontSize: "20px" } : { marginLeft: "8px" }}>Go Back</span>
        </div>
    }

    return <>
        {showStakedIcon && <Box mt={10}>
            {isAuthenticated && <>
                {isChildrenPage ? <Box ml={4}><GoBackComponent /></Box> : <Box ml={5}><Typography style={{ fontSize: "25px", fontWeight: 700 }}>{headerTitle}</Typography></Box>}
            </>}
        </Box>}
    </>
}
